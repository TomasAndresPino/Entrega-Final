...
# Filtrar solo las fallas "4"
filtered_failures = maintenance_data[maintenance_data['Failure_Mode'] == 4]

# Agrupar datos por Machine_ID
tons_per_truck = truck_operational_data.groupby('Machine_ID')['Tons_Loaded'].sum().reset_index()
failures_per_truck = filtered_failures.groupby('Machine_ID')['Failure_Mode'].count().reset_index()
...
plt.ylabel('Cantidad de Fallas (Falla 4)')
plt.title('Scatter Plot: Fallas de Suspensión (4) vs Toneladas Cargadas por Camión')
plt.legend()
plt.grid()
